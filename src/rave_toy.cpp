...
#include "src/lib/dhcpserver.hpp"
#include "src/lib/dnsserver.hpp"
...
#define LED_TEST_BODY "<html><body><h1>Hello from Pico W.</h1><p>Led is %s</p><p><a style=\"font-size: 10rem\" href=\"?led=%d\">Turn led %s</a></body></html>"
#define LED_PARAM "led=%d"
#define LED_TEST "/ledtest"
#define LED_GPIO 0
#define HTTP_RESPONSE_REDIRECT "HTTP/1.1 302 Redirect\nLocation: http://%s" LED_TEST "\n\n"

using namespace rave_toy::config;
using namespace rave_toy::dhcp::server;
using namespace rave_toy::dns::server;
...
namespace rave_toy {
    ...
    static err_t
    tcp_close_client_connection(TCP_CONNECT_STATE_T* con_state, struct tcp_pcb* client_pcb, err_t close_err) {
        if (client_pcb) {
    ...
    static err_t tcp_server_accept(void* arg, struct tcp_pcb* client_pcb, err_t err) {
        ...
        auto con_state = (TCP_CONNECT_STATE_T*) calloc(1, sizeof(TCP_CONNECT_STATE_T));
        if (!con_state) {
...
// This "worker" function is called to safely perform work when instructed by key_pressed_func
    ...
    [[maybe_unused]] int main() {
        ...
        auto state = (TCP_SERVER_T*) calloc(1, sizeof(TCP_SERVER_T));
        if (!state) {
